	...
	public:
		...
		res_append_entry append_entry(req_append_entry args) {
			...
			if (conflict_index == 0) {
				...
				for (auto& entry : v)
				{
					LOG_INFO << "node {id =" << host_id_ << "} append entry " << to_string(entry) << " from node{id=" << args.from << "}";
				}
#endif
		...
		void step_down_follower(uint64_t term) {
			if (state_ == State::CANDIDATE) {
				...
				LOG_INFO << "leader step down to follower ";
		...
		void become_leader() {
			...
			log_.append({ &entry });
			LOG_INFO << "node {id =" << host_id_ << "} append entry " << to_string(entry);
		}
